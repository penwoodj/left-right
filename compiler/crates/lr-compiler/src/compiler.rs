...
impl Compiler {
    ...
    fn compile_async_expression(&mut self, a: &AsyncExpression, dest: u8) -> Result<(), CompilerError> {
        self.compile_expression(&a.operator, dest)?;
        self.chunk.emit(Instruction::new(Opcode::MakeAsync, dest, 0, 0));
        Ok(())
    }

    fn compile_await_expression(&mut self, a: &AwaitExpression, dest: u8) -> Result<(), CompilerError> {
        self.compile_expression(&a.promise, dest)?;
        self.chunk.emit(Instruction::new(Opcode::Await, dest, 0, 0));
        Ok(())
    }
